...
| `_TZ3210_ol1uhvza` | `TS130F` | Lonsonho QS-Zigbee-C03 roller-shutter / curtain module. Keeps the upstream position fix and adds the Tuya settings as entities: **Motor reversal** switch (swap up/down), **Calibration mode** switch, **Travel time** number (seconds, settable to the tenth the device's auto-calibration produces), plus **Moving state** (Up/Stop/Down) and **Opening** (%) diagnostic sensors. **Fixes the position jumping back to a fixed value when the shutter is idle**: on this firmware `current_position_lift_percentage` is a writable NVRAM register, not a live read-out, so ZHA's 15-minute periodic report (and its read on startup) answer with whatever was last written to it. The quirk ignores position updates that arrive while the motor is at rest and contradict the known position, and writes the true position back into the device a few seconds after every travel, so reads and reports agree with reality and the position survives a restart. |
...
| `tests/ts130f_quirk_test.py` | `python3 tests/ts130f_quirk_test.py` | The TS130F stale-position guard, its write-back, and the travel-time scaling. |
