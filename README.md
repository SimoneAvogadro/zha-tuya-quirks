...
Besides the quirks, the integration registers two radio-level helper services. They exist because some Tuya devices need help that only a real Zigbee frame can give, and the platform-agnostic irrigation integration in [tuya-cards-for-ha](https://github.com/SimoneAvogadro/tuya-cards-for-ha) calls them instead of importing ZHA internals. Both take `entity_id`: any entity of the target ZHA device (the valve switch, typically). Both raise a normal service error if the entity is not a ZHA device.

| Service | What it does |
|---|---|
| `zha_tuya_quirks.push_device_time` | Emits the Tuya MCU time-sync command (0x24) on the device's 0xEF00 cluster with the current time, using the quirk's epoch. The GiEX QT06 RTC drifts and the firmware never asks for a sync, so without this its `irrigation_start_time` / `irrigation_end_time` are wrong. The irrigation integration calls it right before opening the valve. |
